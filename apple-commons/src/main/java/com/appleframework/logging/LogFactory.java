...
import com.appleframework.logging.dialect.console.ConsoleLogFactory;
import com.appleframework.logging.dialect.slf4j.Slf4jLogFactory;
import com.appleframework.tools.io.resource.ResourceUtil;
import com.appleframework.tools.lang.caller.CallerUtil;
import com.appleframework.tools.util.ServiceLoaderUtil;
...
public abstract class LogFactory {
	...
	private static LogFactory doCreate() {
		...
		// 未找到任何可支持的日志库时判断依据：当Logback Logging的配置文件位于classpath中，使用slf4j Logging，否则使用Console
		final URL url = ResourceUtil.getResource("logback.xml");
		return (null != url) ? new Slf4jLogFactory() : new ConsoleLogFactory();
	}
	// ------------------------------------------------------------------------- Static end
}
