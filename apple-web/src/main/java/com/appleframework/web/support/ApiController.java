...
import com.appleframework.web.ParamNames;
import com.appleframework.web.RespWriter;
import com.appleframework.web.Result;
import com.appleframework.web.auth.Oauth2Manager;
import com.appleframework.web.auth.Oauth2Service;
import com.appleframework.web.bean.ApiSearch;
import com.appleframework.web.bean.Consts;
import com.appleframework.web.bean.RequestMode;
...
import com.appleframework.web.limit.LimitSearch;
import com.appleframework.web.message.ErrorFactory;
import com.appleframework.web.message.Errors;
import com.appleframework.web.register.AbstractInitializer;
import com.appleframework.web.template.DocTemplate;
import com.appleframework.web.template.HandshakeTemplate;
import com.appleframework.web.template.InvokeTemplate;
import com.appleframework.web.template.LimitTemplate;
import com.appleframework.web.template.MonitorTemplate;
import com.appleframework.web.template.StatusParam;
import com.appleframework.web.template.WebfluxInvokeTemplate;
import com.appleframework.web.util.BeanUtil;
...
public abstract class ApiController extends AbstractInitializer implements ApplicationListener<ContextRefreshedEvent> , ResponseHandler, VelocityContextHandler {
    ...
    public Result caugthException(Throwable e) {
		Integer code = Errors.SYS_ERROR.getCode();
		String module = Errors.OPEN_ISV;
		...
		if (e instanceof ApiException) {
			...
			data = apiEx.getData();
		} else if (e.getClass().equals(apiConfig.getServiceExceptionClass())) {
			try {
				code = (Integer)BeanUtil.invokeGet(e, "code");
				Object obj = BeanUtil.invokeGet(e, "message");
				if(null != obj) {
					message = (String)obj;
				}
			} catch (Exception e1) {
			...
			if(null == message) {
				try {
					module = (String)BeanUtil.invokeGet(e, "module");
				} catch (Exception e1) {
				}
			}
		}
		...
		if(StringUtils.isEmpty(message)) {
			message = ErrorFactory.getErrorMessage(module + code + "", ApiContext.getLocal(), "");
		} else {
			if (message.startsWith(LEFT_TOKEN) && message.endsWith(RIGHT_TOKEN)) {
				module = message.substring(1, message.length() - 1);
				message = ErrorFactory.getErrorMessage(module, ApiContext.getLocal(), "");
			}
		}
		return this.apiConfig.getResultCreator().createErrorResult(code, message, data);
	}
    ...
    @ResponseBody
    public Map<String, Object> jsondoc(HttpServletRequest request, HttpServletResponse response) throws Throwable {
        ...
        context.put("SYSTEM_PARAM_NAMES", ParamNames.SYSTEM_PARAM_NAMES);
    ...
    @ResponseBody
    public Map<String, Object> docPwd(String password, HttpServletRequest request) throws Throwable {
    ...
    public void jsonErrorHandler(HttpServletRequest request, HttpServletResponse response, Throwable e) {
        ...
        try {
            ...
            logger.error("com.appleframework.web.support.ApiController.jsonErrorHandler()写json失败", e1);
        }
    }
