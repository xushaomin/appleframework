...
import com.appleframework.web.auth.impl.Oauth2ServiceImpl;
...
import com.appleframework.web.monitor.ApiMonitorStore;
import com.appleframework.web.monitor.MonitorInterceptor;
import com.appleframework.web.monitor.MonitorStore;
import com.appleframework.web.permission.PermissionManager;
import com.appleframework.web.doc.DocFileCreator;
import com.appleframework.web.serializer.JsonResultSerializer;
import com.appleframework.web.serializer.XmlResultSerializer;
import com.appleframework.web.session.ApiSessionManager;
import com.appleframework.web.session.SessionManager;
...
public class ApiConfig {
     ...
     * 参数解析
     */
    private ParamParser paramParser2 = new ApiParamParser2();
    ...
    private Class<?> serviceExceptionClass;
	...
	public void setServiceException(String serviceException) {
		...
		if (null != serviceException) {
			try {
				serviceExceptionClass = Class.forName(serviceException);
			} catch (ClassNotFoundException e) {
			}
		}
	}

	public Class<?> getServiceExceptionClass() {
		return serviceExceptionClass;
	}

	public void setServiceExceptionClass(Class<?> serviceExceptionClass) {
		this.serviceExceptionClass = serviceExceptionClass;
	}

	public ParamParser getParamParser2() {
		return paramParser2;
	}

	public void setParamParser2(ParamParser paramParser2) {
		this.paramParser2 = paramParser2;
	}
	
    public void setTraceId(String traceId) {
        ParamNames.TRACE_ID = traceId;
        ParamNames.SYSTEM_PARAM_NAMES.add(traceId);
    }
        
    public void setSystemParamNames(String... paramName) {
    	for (int i = 0; i < paramName.length; i++) {
			if(StringUtils.isNotBlank(paramName[i])) {
				 ParamNames.SYSTEM_PARAM_NAMES.add(paramName[i]);
			}
		}
    }
