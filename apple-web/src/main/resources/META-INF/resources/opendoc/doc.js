...
function request($self, _parent, success, error) {
    ...
    try {
        ...
        var systemParamMap = new Map();
        $(".system-param-names").each(function(i,e){
        	var nKey = $(this).attr("id");
        	var nValue = $(this).val();
        	systemParamMap.set(nKey, nValue)
     	});
        
        //systemParamMap.forEach(function (item, key, mapObj) {
        ...
        sdk.config({
            ...
            data_name: dataName,
            api_name: apiName,
            system_param_map: systemParamMap,
        });
        sdk.post({
            name: name // 接口名
            ,version:version
